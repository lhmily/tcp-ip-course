...
static void test_rejections(tcpip_test_context *ctx) {
  ...
  TCPIP_EXPECT_U32(
      ...
      TCPIP_L02_TRUNCATED);
  TCPIP_EXPECT_SIZE(ctx, ethernet.payload_length, 0U);
  ...
  TCPIP_EXPECT_U32(
      ...
      tcpip_l02_parse_arp(tcpip_l02_test_frame + TCPIP_L02_ETHERNET_HEADER_LENGTH,
                          ...
                          &arp),
      TCPIP_L02_TRUNCATED);
  TCPIP_EXPECT_U16(ctx, arp.operation, 0U);
...
static void test_capacity_is_atomic(tcpip_test_context *ctx) {
  uint8_t output[TCPIP_L02_ARP_REQUEST_FRAME_LENGTH];
