...
First compute an absolute deadline from `CLOCK_MONOTONIC`; wall-clock changes must not alter elapsed-time behavior. Before each I/O attempt, subtract the current monotonic time and round the remaining interval up to milliseconds for `poll`. Retry `poll`, `send`, `recv`, and `accept` when interrupted by `EINTR`. Advance the buffer offset only after a positive result. A zero-length read before any requested byte is `TCPIP_L08_EOF`; after partial progress it is `TCPIP_L08_TRUNCATED`. If the deadline expires, return `TCPIP_L08_TIMEOUT`. Other POSIX failures become `TCPIP_L08_SYSTEM`.
